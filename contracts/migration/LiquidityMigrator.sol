...
contract LiquidityMigrator is AxodusAccessControlled {
    ...
    IERC20 internal immutable oldAXDS = IERC20(0x383518188C0C6d7730D91b2c03a03C837814a899);
    IERC20 internal immutable newAXDS = IERC20(0x64aa3364F17a4D01c6f1751Fd97C2BD3D7e7f1D5);
    ...
    function migrateLP(
        ...
        uint256 initialNewAXDSBalance = newAXDS.balanceOf(address(this));
        ...
        (uint256 amountToken, uint256 amountAXDS) = routerFrom.removeLiquidity(
            token,
            address(oldAXDS),
            oldLPAmount,
            _minA,
        ...
        // Migrate the V1 AXDS to V2 AXDS
        oldAXDS.approve(address(migrator), amountAXDS);
        migrator.migrate(amountAXDS, IMigrator.TYPE.UNSTAKED, IMigrator.TYPE.UNSTAKED);
        uint256 amountNewAXDS = newAXDS.balanceOf(address(this)).sub(initialNewAXDSBalance); // # V1 out != # V2 in

        // Add the V2 liquidity
        IERC20(token).approve(address(routerTo), amountToken);
        newAXDS.approve(address(routerTo), amountNewAXDS);
        ...
        newAXDS.safeTransfer(authority.governor(), newAXDS.balanceOf(address(this)));
        oldAXDS.safeTransfer(authority.governor(), oldAXDS.balanceOf(address(this)));
