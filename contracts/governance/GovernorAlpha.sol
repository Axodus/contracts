...
contract GovernorAlpha {
    ...
    /// @notice change from original contract
    function proposalThresholdPercent() public pure returns (uint) { return 10000; } // 1.00% of sAXDS circulating supply : In ten-thosandaths 10000 = 1.00%
    ...
    /// @notice The address of the Axodus Protocol Timelock
    TimelockInterface public timelock;

    /// @notice The address of the gAXDS token
    /// @notice change from original contract
    gAXDSInterface public gAXDS;

    /// @notice The address of the sAXDS token
    /// @notice change from original contract
    sAXDSInterface public sAXDS;
    ...
    struct Proposal {
        ...
        /// @notice Threshold of gAXDS at start of proposal
        /// @notice change from original contract
        uint thresholdAtStart;

        /// @notice Number of gAXDS needed to pass vote
        /// @notice change from original contract
        uint votesNeeded;
    ...
    constructor(address timelock_, address sAXDS_, address gAXDS_, address guardian_ ) public {
    ...
    function propose(address[] memory targets, uint[] memory values, string[] memory signatures, bytes[] memory calldatas, string memory description) public returns (uint) {
        /// @notice change from original contract
        require(gAXDS.getPriorVotes(msg.sender, sub256(block.number, 1)) > getVotesFromPercentOfsAXDSSupply(proposalThresholdPercent()), "GovernorAlpha::propose: proposer votes below proposal threshold");
        ...
        Proposal memory newProposal = Proposal({
            ...
            againstVotes: 0,
            /// @notice change from original contract
            thresholdAtStart: getVotesFromPercentOfsAXDSSupply(proposalThresholdPercent()),
            /// @notice change from original contract
    ...
    function cancel(uint proposalId) public {
        ...
        Proposal storage proposal = proposals[proposalId];
        /// @notice change from original contract
        require(msg.sender == guardian || gAXDS.getPriorVotes(proposal.proposer, sub256(block.number, 1)) < proposal.thresholdAtStart, "GovernorAlpha::cancel: proposer above threshold");
